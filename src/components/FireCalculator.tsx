        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-4 sm:p-6 rounded-2xl shadow-lg border border-slate-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div className="space-y-2">
                    <label className="text-xs sm:text-sm font-medium text-slate-300">Annual Withdrawal Target</label>
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-7 pr-3 py-2 rounded-lg border border-slate-600 bg-slate-900/50 text-sm font-mono focus:ring-2 focus:ring-green-500 outline-none"
                        />
                    </div>
                ...
                <div className="space-y-2">
                    <div className="text-xs sm:text-sm text-slate-400">Current Stack Value</div>
                ...
                {scenarioResults.map((result) => (
                    <div
                        ...
                        className={clsx(
                            ...
                            result.label === 'Conservative' && "bg-blue-900/20 border-blue-700/50",
                            result.label === 'Moderate' && "bg-green-900/20 border-green-700/50",
                            result.label === 'Aggressive' && "bg-amber-900/20 border-amber-700/50",
                        )}
                    >
                        <div className="text-xs font-medium text-slate-300 mb-2">{result.label} ({(result.rate * 100).toFixed(0)}%/yr)</div>
                        <div className="text-xl sm:text-2xl font-bold">
                            {result.reached ? (
                                <span className="text-green-400">{result.years === 0 ? 'Now' : `${result.years}y`}</span>
                            ) : (
                                <span className="text-red-400">100y+</span>
                            )}
                        </div>
            ...
            <div className="mt-3 text-[10px] sm:text-xs text-slate-500 text-center">
