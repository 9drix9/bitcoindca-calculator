    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
            ...
            <div className="text-center mb-3">
                <span className={`text-2xl sm:text-3xl font-bold tabular-nums ${getValueColor(initialData.value)}`}>
                    {initialData.value}
                </span>
                <span className="text-xs sm:text-sm text-slate-400 ml-0.5">/100</span>
                <p className={`text-xs sm:text-sm font-medium mt-0.5 ${getValueColor(initialData.value)}`}>
            ...
            <div className="relative h-2.5 rounded-full overflow-hidden bg-gradient-to-r from-red-500 via-yellow-500 to-green-500">
                <div
                    ...
                    style={{ left: `${initialData.value}%`, transform: 'translateX(-50%)' }}
                />
            </div>
